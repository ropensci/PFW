...
Once you've filtered your data to your liking, you'll want to zerofill it before starting any analyses. Because Project FeederWatch data relies on counts of birds detected, there is no data when a bird is not detected, which is required for presence/absence analysis. For example, a survey with 20 Lesser Goldfinches on it will say just that-- `HOW_MANY == 20`-- but a survey with no Lesser Goldfinches on it will have no data instead of `HOW_MANY == 0`. 
...
Maron, M. W., E. I. Greig, and J. Boersma (2025). Climate and Landscape Modification Facilitate Range Expansion in *Spinus psaltria* (Lesser Goldfinch) across the Pacific Northwest. Ornithology (In Press).
